...
public class DijkstraGraph<NodeType, EdgeType extends Number>
    ...
    protected SearchNode computeShortestPath(NodeType start, NodeType end) {
        ...
        MapADT<Node, Integer> visited = new PlaceholderMap<>();
        ...
        while (!priorityQueue.isEmpty()) {
            ...
            if (current.node.data == nodes.get(end).data) {
                return current;
            }
    ...
    public List<NodeType> shortestPathData(NodeType start, NodeType end) {
        // use the searchNode provided by the helper method to trace to the starting node
        SearchNode searchNode = computeShortestPath(start, end);
        List<NodeType> nodeList = new ArrayList<>();
        
        // add the nodes to a list
        while (searchNode.node.data != nodes.get(start).data) {
